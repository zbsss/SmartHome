...
my_range = {"name": "c1", # name of the circuit
            "floor": "f1",
            "rooms": [
                {
                    "name": "living room",
...
def local(operation='toggle'):
    ...
    send(f"{my_range['name']};f1;living room;lamp;1;{'on' if led1.is_active else 'off'}")
...
def non_local(operation='toggle'):
    ...
    if operation == 'on':
        ...
        send(f"{my_range['name']};f1;kitchen;lamp;1;toggle")
...
@circuit.run
def main():
    button1.when_pressed = local
    button2.when_pressed = non_local
