...
my_range = {"name": "c5", # name of the circuit
            "floor": "f1",
            "rooms": [
                {
                    "name": "bathroom",
                    "devices": [
                        ...
                        {"id": '1', "type": "mirror lamp", "local": True}
                    ]
                }
            ]}
...
def local(led, id, type):
    def local_specific(operation='toggle'):
        ...
        send(f"{my_range['name']};f1;bathroom;{type};{id};{'on' if led.is_active else 'off'}")
...
@circuit.run
def main():
    button1.when_pressed = local(led1, 1, 'lamp')
    button2.when_pressed = local(led2, 2, 'mirror lamp')
